    ...
    <header className="sticky top-0 left-0 right-0 z-50 shadow-md backdrop-blur-md font-sans">
      ...
      <div className="bg-white border-b-2 border-amber-400">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 h-14 flex items-center justify-between gap-3">
          ...
          {/* STICKY MAIN DEPARTMENT NAVIGATION - DYNAMICALLY FILTERED BY ROLE */}
            ...
            {visibleMainNav.map((dept) => {
              ...
              return (
                <Link
                  key={dept.name}
                  href={dept.href}
                  className={`flex items-center gap-1.5 px-3 sm:px-4 py-1.5 rounded-xl text-xs sm:text-sm font-extrabold transition-all whitespace-nowrap ${
                    isActive
      ...
      {isDrawerOpen && (
        ...
        >
          <div
            className="absolute top-24 right-4 w-72 bg-white border-2 border-blue-600 rounded-2xl p-4 space-y-3 shadow-2xl"
